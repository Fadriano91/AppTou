...
function ListaPosicao({ data, navigation, theme }) {
    const { colors } = theme
    ...
    async function excluirPosicao(data) {
        let url = `${BACKEND}/posicoes/${data._id}`
        await fetch(url, {
            method: 'DELETE',
            headers: {
            ...
            .catch(function (error) {
                console.error('Houve um problema ao escluir a posição:' + error.message);
            })
    }

    const alteraPosicao = async (data) => {
        navigation.navigate('AdicionaPosicao', {
            data: data
        })
    }
...
export default withTheme(ListaPosicao)